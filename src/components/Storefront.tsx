...
export function Storefront() {
  ...
  const handleOrder = (item: InventoryItem) => {
    if (item.quantityStocked <= 0) return;
    
    const message = `Hi, I want to buy the ${item.itemName} in size ${item.size}. is it available?`;
        ...
        {filteredItems.length > 0 ? (
          <div className="grid grid-cols-2 gap-x-4 gap-y-12 sm:grid-cols-3 lg:grid-cols-4 sm:gap-x-6 lg:gap-x-8">
            {filteredItems.map((item) => {
              const isSoldOut = item.quantityStocked <= 0;
                  ...
                  <div className="flex flex-col flex-1">
                    ...
                    <div className="mt-auto flex flex-col gap-4">
                      <div className="flex justify-between items-end">
                        ...
                        <p className="text-xs text-gray-500">Available: <span className="font-medium text-black">{item.size}</span></p>
                      </div>
